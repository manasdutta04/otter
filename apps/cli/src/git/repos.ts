...
import { loadConfig, requireAuth, reposRoot } from "../config.js";
import {
  getLinkedRepoId,
  getRepository,
  linkProject,
...
export function resolveWorkRoot(explicitPath?: string): { root: string; repositoryId?: string } {
  if (explicitPath) {
    return { root: path.resolve(explicitPath) };
  }
...
export async function createPullRequest(opts: {
  ...
  await git.commit(`otter: ${opts.title}`);
  await git.push(
    `https://x-access-token:${auth.accessToken}@github.com/${opts.owner}/${opts.repo}.git`,
    opts.branch,
    ["-u"],
  );
