...
async def analyze_architecture(db: AsyncSession, repository_id: str, root: Path) -> RepositoryArchitectureAnalysis:
    ...
    record = RepositoryArchitectureAnalysis(repository_id=repository_id, score=score, findings=json.dumps(findings), created_at=datetime.now(timezone.utc)); await db.merge(record); await db.commit(); await db.refresh(record); return record
