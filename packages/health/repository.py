...
from app.models import RepositoryHealth
...
async def analyze_health(db: AsyncSession, repository_id: str, root: Path, file_count: int) -> RepositoryHealth:
    ...
    security = 100
    if any(name in {".env", ".env.local", "id_rsa"} for name in names):
        findings.append("Sensitive-looking files are present in the repository tree.")
        security -= 35
    if not any("test" in str(path).lower() for path in files):
        findings.append("No obvious test directory or test file was detected.")
        maintainability = 55
    else:
        maintainability = 80
    if not any(name in {"requirements.txt", "pyproject.toml", "package.json", "go.mod", "cargo.toml"} for name in names):
        findings.append("No conventional dependency manifest was detected.")
        dependency = 50
    else:
        dependency = 80
