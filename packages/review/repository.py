...
from app.models import RepositoryReview
...
def review_repository(root: Path) -> list[dict[str, object]]:
    findings = []
    for path in RepositoryRetrievalIndex(root).files:
        if path.suffix.lower() not in {".py", ".js", ".ts", ".tsx", ".java", ".go"}:
            continue
        try:
        ...
        for category, severity, pattern, title in PATTERNS:
            ...
            if match:
                findings.append({"category": category, "severity": severity, "title": title, "file": relative, "line": content[:match.start()].count("\n") + 1})
    return findings[:100]


async def save_review(db: AsyncSession, repository_id: str, user_id: str, findings: list[dict[str, object]]) -> RepositoryReview:
    record = RepositoryReview(id=uuid4().hex[:12], repository_id=repository_id, user_id=user_id, findings=json.dumps(findings), created_at=datetime.now(timezone.utc))
    db.add(record)
    await db.commit()
