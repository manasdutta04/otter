...
## Phase 5, Step 1 — CLI surface

Added a thin `veridexs` CLI that calls the existing API instead of reimplementing analysis. It supports `health`, `review`, `architect`, `analyze`, `plan`, and `docs` commands, plus configurable API URL and session-cookie authentication through environment variables.

This is the first Phase 5 client surface. The CLI keeps product behavior centralized in the API so the future dashboard, VS Code extension, GitHub App, and MCP server can share the same contracts.
